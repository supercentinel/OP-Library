...
let editBookFromLibrary = function (table, id, book) {
    table.rows[id].cells[1].textContent = book.title;
    table.rows[id].cells[2].textContent = book.author;
    table.rows[id].cells[3].textContent = book.nPages;
    table.rows[id].cells[4].textContent = book.pDate;
    table.rows[id].cells[5].textContent = book.readed;
}

let resortBookId = function (table) {
    for (let i = 1; i < table.rows.length; i++) {
        table.rows[i].cells[0].textContent = i;
    }
}

let removeBookFromLibrary = function (table, id) {
    table.removeChild(table.rows[id]);
    resortBookId(table);
}
...
popUpForm.querySelector("form").onsubmit = function (e) {
    ...
    switch (currentAction) {
        ...
        break;
        case action.EDIT:
            bookNumber = popUpForm.querySelector("#bookNumber").value;

            if (bookNumber > table.rows.length - 1) {
                alert("There is no book with that number");
                return;
            }

            let edditedBook = new Book(
                popUpForm.querySelector("#title").value,
                popUpForm.querySelector("#author").value,
                popUpForm.querySelector("#nPages").value,
                popUpForm.querySelector("#pDate").value,
            ...
            editBookFromLibrary(table, bookNumber, edditedBook);
        break;
        case action.REMOVE:
            bookNumber = popUpForm.querySelector("#bookNumber").value;
            ...
            removeBookFromLibrary(table, bookNumber);
        break;
    }
}
